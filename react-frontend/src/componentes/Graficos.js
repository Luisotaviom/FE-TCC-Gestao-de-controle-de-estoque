...
const Graficos = () => {
  ...
  useEffect(() => {
    const buscarMovimentacoes = async () => {
      try {
        const response = await MovimentacoesDataService.getRelatorioMensal();
        if (isMounted) {
  ...
  const { quantidadesEntradas, quantidadesSaidas } =
    processarQuantidadesProdutos();
  ...
  return (
    <div>
      ...
      <div style={{ height: "500px", width: "100%" }}>
        <Line
          data={dadosGraficoQuantidades}
          options={opcoesGraficoQuantidades}
        />
      </div>
    </div>
  );
